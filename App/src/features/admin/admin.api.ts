...
import type { AdminAnalytics, AdminActivityEntry, AdminRefreshToken, AdminTable } from "./admin.types";
...
export const adminApi = {
  ...
  createPostgresRow: (tableName: string, payload: Record<string, any>) =>
    api.post<Record<string, any>>(`/api/admin/postgres/${tableName}`, payload).then((r) => r.data),

  updatePostgresRow: (tableName: string, id: string, payload: Record<string, any>) =>
    api.put<Record<string, any>>(`/api/admin/postgres/${tableName}/${id}`, payload).then((r) => r.data),
